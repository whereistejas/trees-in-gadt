module Make (Elt : Set.OrderedType) : sig
   ...
   fun elt tree ->
    ...
    | Eq (left, pivot, right) -> begin
        ...
        | cmp when cmp < 0 -> begin
            ...
            | Overflow_left (a, b, c) ->
                Overflow_left (Left (a, b, c), pivot, right)
            | Overflow_right (a, b, c) ->
                Overflow_left (Right (a, b, c), pivot, right)
          end
        | _ -> begin
            ...
            | Overflow_left (a, b, c) ->
                Overflow_right (left, pivot, Left (a, b, c))
            | Overflow_right (a, b, c) ->
                Overflow_right (left, pivot, Right (a, b, c))
          end
      end
  ...
  let left_left : type h.
      h succ remove_result -> elt -> h tree -> h succ succ remove_result =
   fun left pivot right ->
    match left with
  ...
  let right_left : type h.
      h remove_result -> elt -> h succ tree -> h succ succ remove_result =
   fun left pivot right ->
  ...
  let eq_right : type h.
      h tree -> elt -> h remove_result -> h succ remove_result =
   fun left pivot right ->
    match right with
    | Uk right -> Uk (Eq (left, pivot, right))
    | Underflow right -> Uk (Left (left, pivot, right))

  let left_right : type h.
      h succ tree -> elt -> h remove_result -> h succ succ remove_result =
   fun left pivot right ->
    match (left, pivot, right) with
  ...
  let right_right : type h.
      h tree -> elt -> h succ remove_result -> h succ succ remove_result =
   fun left pivot right ->
    match right with
  ...
  let remove elt (T t) =
    match remove elt t with Uk t -> T t | Underflow t -> T t
end
